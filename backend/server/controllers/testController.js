...
const PHONEME_API_URL = process.env.PHONEME_API_URL || 'http://localhost:5002';
  ...
  try {
    ...
    if (!letter || !word || !pronunciation || accuracy === undefined) {
      return res.status(400).json({
        success: false,
        message: 'Please provide letter, word, pronunciation, and accuracy'
      });
    }
...
export const getUserStatistics = async (req, res) => {
  try {
    ...
    const statistics = {
      ...
      inProgressTests: tests.filter(t => !t.completed && t.attempts.length > 0).length,
    ...
    if (tests.length > 0) {
      ...
      statistics.averageOverallAccuracy = Math.round((totalAccuracy / tests.length) * 100) / 100;
      
      const sortedByAccuracy = [...tests].sort((a, b) => b.averageAccuracy - a.averageAccuracy);
      if (sortedByAccuracy[0] && sortedByAccuracy[0].attempts.length > 0) {
      ...
      const withAttempts = sortedByAccuracy.filter(t => t.attempts.length > 0);
